...
const showNumber = (pos) => {
    if (!useNumberInBingo(randNumbers.value[randNumbers.value.length - 1])) {
...
const bingoDone = computed(() => {
    return (
        ...
        bingoSheet.value.every((cell) => cell !== 0)
    )
})
</script>
...
<template>
    <div class="top-container">
        ...
        <div class="grid-container">
            <div class="grid-item text-xs" v-for="(cell, index) in bingoSheet" :key="index">
                <!-- show bingo 3 rows x 3 cols here -->
                <button
                    v-if="cell === 0"
                    :disabled="randNumbers.length === 0"
                    @click="showNumber(index)"
                    class="border text-sm bg-white"
                >Add Number</button>
